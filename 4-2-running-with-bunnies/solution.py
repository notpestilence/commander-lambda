...
def BellmanFord(times, time_limit):
    ...
    # Initialization
    n = len(times)
    d = [n*[sys.maxsize] for _ in range(n)]
    for s in range(n):
        d[s][s] = 0
        # Relaxation
        for i in range(n-1):
...
def solution(times, time_limit):
    ...
    if nc:
        ...
        s = [[0,[0],time_limit,[[i] for i in range(n)]]]
        vertices = set([i for i in range(n)])
        maxBunnies = set()
        maxNumberBunnies = 0
        while s:
            [u,path,timeleft, voidvertices] = s.pop()
            for v in vertices - set(voidvertices[u]):
                ...
                if timeleft-timeuv-timeub >= 0:
                    ...
                    s.append([v,nextPath,nextTimeLeft,nextVoidVertices])
                    if v == n-1: 
